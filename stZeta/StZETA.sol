    ...
    PausableUpgradeable
{
    ...
    /// @notice Daily annual percentage rate (APR), with two decimal places, represented as a percentage with a total of 5 digits
    /// uint256 version apr to avoid precision loss
    uint256 public apr256;
    ...
    function getValidEpoch() external view returns(uint256) {
        ...
        if (smallestEpoch == type(uint256).max) {
            smallestEpoch = 0;
        }
    ...
    function setApr(uint256 _newApr) 
        external
        override
        onlyRole(ORACLE_ROLE) {
        uint256 oldApr = apr256;
        apr256 = _newApr;
    ...
    /// @notice Set the total stakers
    /// @param _newTotalStakers - The new total stakers
    function setTotalStakers(uint256 _newTotalStakers) 
        external override onlyRole(DAO) {
        totalStakers = _newTotalStakers;
        emit SetTotalStakers(_newTotalStakers);
    }

    /// @notice Add the total stakers
    /// @param _addAmount Amount to add
    function addTotalStakers(uint256 _addAmount) 
        external override onlyRole(DAO) {
        totalStakers += _addAmount;
        emit SetTotalStakers(totalStakers);
    }

    /// @notice Subtract the total stakers
    /// @param _subAmount Amount to subtract
    function subTotalStakers(uint256 _subAmount) 
        external override onlyRole(DAO) {
        totalStakers -= _subAmount;
        emit SetTotalStakers(totalStakers);
    }
