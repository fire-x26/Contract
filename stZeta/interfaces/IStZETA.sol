...
interface IStZETA is IERC20Upgradeable {
    ...
    /// @param _newTotalStakers New total stakers
    function setTotalStakers(uint256 _newTotalStakers) external;

    /// @notice Add the total stakers
    /// @param _addAmount Amount to add
    function addTotalStakers(uint256 _addAmount) external;

    /// @notice Subtract the total stakers
    /// @param _subAmount Amount to subtract
    function subTotalStakers(uint256 _subAmount) external;
    ...
    /// @param oldApr old.
    /// @param newApr new.
    event SetApr(uint256 oldApr, uint256 indexed newApr);
    ...
    /// @notice Emitted when total stakers is set.
    /// @param _newTotalStakers new total stakers.
    event SetTotalStakers(uint256 _newTotalStakers);
